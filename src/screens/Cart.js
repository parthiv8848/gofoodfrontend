...
export default function Cart() {
  ...
  const handleCheckOut = async () => {
    ...
    let response = await fetch("https://foodorder-yb5u.onrender.com/api/auth/orderData", {
      // credentials: 'include',
          ...
          <tbody>
            {data.map((food, index) => (
              <tr  >
                ...
                <td ><button type="button" className="btn p-0"     style={{ backgroundColor: 'white' }} ><Delete onClick={() => { dispatch({ type: "REMOVE", index: index }) }  } /></button> </td></tr>
            ))}
          </tbody>
        </table>
